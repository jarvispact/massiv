...
const ExpressAdapter = class {
    ...
    async setupHandlers(fileList) {
        ...
        fileList.forEach(({ method, route, handler }) => {
            ...
            this.framework[method](route, async (req, res) => {
                ...
                if (handler) {
                    try {
                        const result = await handler({ params, query, headers, body, token, logger });
    ...
    async start() {
        const { host, port } = this.config.service;
        this.server = this.framework.listen(port, host, () => {
