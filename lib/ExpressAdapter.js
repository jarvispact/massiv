...
const ExpressAdapter = class {
    ...
    shouldUseAuth(method, route) {
        ...
        for (const disabledRoute of disabledRoutes) {
            ...
            const routeRegexes = routesFromConfig.map(r => pathToRegexp(r));
            const routeMatches = routeRegexes.map(rgxp => rgxp.exec(route));
        ...
        const oneOrMoreMatches = matchList.some(m => m);
        return (hasAuthSetting && !oneOrMoreMatches) || false;
    }
    ...
    setupHandlers(handlerList) {
        ...
        if (config.cors) {
            this.framework.use((req, res, next) => {
                if (config.cors.origins) res.append('Access-Control-Allow-Origin', config.cors.origins);
                if (config.cors.methods) res.append('Access-Control-Allow-Methods', config.cors.methods.join(','));
                if (config.cors.headers) res.append('Access-Control-Allow-Headers', config.cors.headers.join(' '));
                next();
            });
        }
