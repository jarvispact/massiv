...
const ExpressAdapter = class {
    ...
    async aclCheck({ token, route, method, headers, params, query, body, requestId, logger }) {
        ...
        if (aclConfig && aclConfig.routes) {
            for (const routeConfig of aclConfig.routes) {
                ...
                if (oneOrMoreRoutesMatched && methodMatches) {
                    const error = routeConfig.fn ? await routeConfig.fn({ token, headers, params, query, body, requestId, logger }) : null;
                    return error;
                }
            }
        }
    ...
    setupHandlers(handlerList) {
        ...
        handlerList.forEach(({ method, route, handler }) => {
            ...
            this.framework[method](route, async (req, res) => {
                ...
                if (ACLError) {
                    logger.debug(`acl check failed at ${routeInfo}. error message: `, ACLError.message);
