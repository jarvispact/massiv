...
const ExpressAdapter = class {
    ...
    setupHandlers(handlerList) {
        ...
        handlerList.forEach(({ method, route, handler }) => {
            ...
            const useAuthRouteSetting = handler.config ? handler.config.auth !== !1 : true;
            const useAuth = (config.auth && useAuthRouteSetting) || false;
            ...
            this.framework[method](route, async (req, res) => {
                ...
                if (isFunction(handler)) {
                    try {
                        ...
                        logger.debug(`${routeInfo} called. statusCode: ${status}`);
